...
class Prompt {
  ...
  constructor(model, question) {
    ...
    if (this.question.type === 'checkbox') {
      this.transitions = Math.pow(2, this.question.choices.realLength);
  ...
  run() {
    this.questionState = this.model.currentQuestion(this.question.name);
  ...
  _answer(answer) {
    ...
    // this.questionNode.value.answer = value;
